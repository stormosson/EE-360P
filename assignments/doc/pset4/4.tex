...
\item \textbf{(20 points)}
  \begin{enumerate}[(a)]
  \item Extend Lamport's mutex algorithm to solve $k$-mutual exclusion problem
    which allows at most $k$ processes to be in the critical section concurrently.
    ...
    \item \emph{Notes}: Each process keeps track of a counter $c$, representing the
      ...
      to $k$.
    \item To request the critical section, a process sends a timestamped message to
      all other processes and adds a timestamped request to the queue.
    \item On receiving a request message, the request and its timestamp are stored
      in the queue and a timestamped acknowledgement is sent back.
    \item To release the critical section, the process increments $c$ and sends a
      release message to all other processes.
    \item On receiving a release message, the corresponding request is deleted from
      the queue.
    \item A process determines that it can access the critical section if and only
